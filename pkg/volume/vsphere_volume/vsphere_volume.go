...
type vdManager interface {
	// Creates a volume
	CreateVolume(provisioner *vsphereVolumeProvisioner, selectedAvailabilityZone []string) (volSpec *VolumeSpec, err error)
	// Deletes a volume
...
func (v *vsphereVolumeProvisioner) Provision(selectedNode *v1.Node, allowedTopologies []v1.TopologySelectorTerm) (*v1.PersistentVolume, error) {
	if !util.AccessModesContainedInAll(v.plugin.GetAccessModes(), v.options.PVC.Spec.AccessModes) {
        ...
        selectedAvailabilityZones, err := util.SelectZonesForVolume(false, false, "", nil, nil, selectedNode, allowedTopologies, v.options.PVC.Name, 1)
        klog.V(4).Infof("Selected zones for volume : %s", selectedAvailabilityZones)
	volSpec, err := v.manager.CreateVolume(v, selectedAvailabilityZones.List())
	if err != nil {
		return nil, err
	}
