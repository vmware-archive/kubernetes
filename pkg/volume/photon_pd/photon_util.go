...
func (util *PhotonDiskUtil) CreateVolume(p *photonPersistentDiskProvisioner) (pdID string, capacityGB int, err error) {
	...
	if err != nil {
		glog.Errorf("Photon Controller utility: CreateVolume failed to get cloud provider")
		return "", 0, err
	}
	...
	if err != nil {
		glog.Errorf("Photon Controller utility: failed to CreateDisk, error [%v]", err)
		return "", 0, fmt.Errorf("Photon Controller utility: failed to CreateDisk, error [%v]", err)
	}
...
func (util *PhotonDiskUtil) DeleteVolume(pd *photonPersistentDiskDeleter) error {
	...
	if err != nil {
		glog.Errorf("Photon Controller utility: DeleteVolume failed to get cloud provider")
		return err
	}

	if err = cloud.DeleteDisk(pd.pdID); err != nil {
		glog.Errorf("Photon Controller utility: failed to DeleteDisk for pdID %s, error [%v]", pd.pdID, err)
		return fmt.Errorf("Photon Controller utility: failed to DeleteDisk for pdID %s, error [%v]", pd.pdID, err)
	}
...
func getCloudProvider(cloud cloudprovider.Interface) (*photon.PCCloud, error) {
	if cloud == nil {
		glog.Errorf("Photon Controller utility: Cloud provider not initialized properly")
		return nil, fmt.Errorf("Photon Controller utility: Cloud provider not initialized properly")
	}
