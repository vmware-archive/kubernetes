...
func (attacher *photonPersistentDiskAttacher) WaitForAttach(spec *volume.Spec, devicePath string, timeout time.Duration) (string, error) {
	...
	for {
		select {
		case <-ticker.C:
			glog.V(5).Infof("Checking PD %s is attached", volumeSource.PdID)
			...
			if err != nil {
				...
				glog.Infof("Successfully found attached PD %s.", volumeSource.PdID)
...
func (detacher *photonPersistentDiskDetacher) Detach(deviceMountPath string, nodeName types.NodeName) error {
	...
	if err == nil && !attached {
		...
		glog.Infof("detach operation was successful. persistent disk %q is already detached from node %q.", pdID, hostName)
		return nil
	}
