...
func getDatastoresForZone(ctx context.Context, dc *vclib.Datacenter, nodeManager *NodeManager, selectedAvailabilityZones []string) ([]*vclib.DatastoreInfo, error) {

        var sharedDatastores []*vclib.DatastoreInfo

        // Create cloudprovider.Zone type from selectedAvailabilityZones
        var cpZones []cloudprovider.Zone
        for _, selectedAvailabilityZone := range selectedAvailabilityZones  {
               var cpZone cloudprovider.Zone
               cpZone.FailureDomain = selectedAvailabilityZone
        ...
        for _, cpZone := range cpZones {
		 ...
		 if len(sharedDatastoresPerZone) == 0 {
			sharedDatastoresPerZone = dsObjList
                        ...
                        klog.V(1).Infof("left sharedDatastoresPerZone : %s", sharedDatastoresPerZone)
                        klog.V(1).Infof("right dsObjList : %s", dsObjList)
			sharedDatastoresPerZone = intersect(sharedDatastoresPerZone, dsObjList)
              ...
              klog.V(1).Infof("Shared datastore per zone %s is %s", cpZone, sharedDatastoresPerZone)
...
func getPbmCompatibleZonedDatastore(ctx context.Context, dc *vclib.Datacenter, storagePolicyName string, datastores []*vclib.DatastoreInfo) (string, error) {
        pbmClient, err := vclib.NewPbmClient(ctx, dc.Client())
        if err != nil {
                return "", err
        }
       ...
       if err != nil {
        ...
        compatibleDatastores, _, err := pbmClient.GetCompatibleDatastores(ctx, dc, storagePolicyID, datastores)
        if err != nil {
                klog.Errorf("Failed to get compatible datastores from datastores : %+v with storagePolicy: %s. err: %+v",
                        datastores, storagePolicyID, err)
                return "", err
        }
        klog.V(9).Infof("compatibleDatastores : %+v", compatibleDatastores)
        datastore, err := getMostFreeDatastoreName(ctx, dc.Client(), compatibleDatastores)
