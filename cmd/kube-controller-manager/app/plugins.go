...
import (
	...
	"k8s.io/kubernetes/pkg/volume/photoncontroller_pd"
...
func ProbeRecyclableVolumePlugins(config componentconfig.VolumeConfiguration) []volume.VolumePlugin {
	...
	allPlugins = append(allPlugins, photoncontroller_pd.ProbeVolumePlugins()...)
...
func NewVolumeProvisioner(cloud cloudprovider.Interface, config componentconfig.VolumeConfiguration) (volume.ProvisionableVolumePlugin, error) {
	...
	case cloud != nil && photoncontroller.ProviderName == cloud.ProviderName():
		return getProvisionablePluginFromVolumePlugins(photoncontroller_pd.ProbeVolumePlugins())
	}
	return nil, nil
}
