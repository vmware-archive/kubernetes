...
func (vs *VSphere) GetVMByUUID(ctx context.Context, vmUUID string, dc object.Reference) (object.Reference, error) {
	...
	//framework.Logf("Unable to find VM by UUID. VM UUID: %s, datacenter: %s, vSphere: %s", vmUUID, dc.Name(), vs.Config.Hostname)
...
func (vs *VSphere) CreateVolume(volumeOptions *VolumeOptions, dataCenterRef types.ManagedObjectReference) (string, error) {
	...
	if err != nil {
		if soap.IsSoapFault(err) {
			...
			if _, ok := soapFault.VimFault().(types.FileAlreadyExists); ok {
				...
				glog.V(4).Infof("Directory with the path %+q is already present", directoryPath)
			}
		}
		if !directoryAlreadyPresent {
			glog.Errorf("Cannot create dir %#v. err %s", directoryPath, err)
			return "", err
		}
	}
	glog.V(4).Infof("Created dir with path as %+q", directoryPath)
	...
	if err != nil {
		glog.Errorf("Failed to create virtual disk: %s. err: %+v", vmdkPath, err)
		return "", err
	}
	taskInfo, err := task.WaitForResult(ctx, nil)
	if err != nil {
		glog.Errorf("Failed to complete virtual disk creation: %s. err: %+v", vmdkPath, err)
		return "", err
	}
...
func (vs *VSphere) IsVMPresent(vmName string, dataCenterRef types.ManagedObjectReference) (isVMPresent bool, err error) {
	...
	if err != nil {
		glog.Errorf("Failed to get children from Folder: %s. err: %+v", vmFolder.InventoryPath, err)
		return
	}
