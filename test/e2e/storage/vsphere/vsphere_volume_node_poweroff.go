...
var _ = utils.SIGDescribe("Node Poweroff [Feature:vsphere] [Slow] [Disruptive]", func() {
	...
	*/
	It("verify volume status after node power off", func() {
		...
		isAttached, err := verifyVSphereDiskAttached(client, nodeInfo.VSphere, volumePath, node1)
		...
		err = waitForVSphereDiskToAttach(client, nodeInfo.VSphere, volumePath, node2)
		...
		err = waitForVSphereDiskToDetach(client, nodeInfo.VSphere, volumePath, node1)
