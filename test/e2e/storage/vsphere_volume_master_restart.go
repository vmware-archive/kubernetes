...
/*
	...
	7. Delete the volume
*/
var _ = SIGDescribe("Volume Attach Verify [Feature:vsphere]", func() {
	...
	It("verify volume remains attached after master kubelet restart", func() {
		...
		for i := 0; i < testIteration; i++ {
			By(fmt.Sprintf("Creating a test vsphere volume %v", i))
			volumePath, err := createVSphereVolume(vsp, nil)
			Expect(err).NotTo(HaveOccurred())

			volumePaths = append(volumePaths, volumePath)

			By(fmt.Sprintf("Creating pod %v on node %v", i, nodeNameList[i]))
			...
			By(fmt.Sprintf("Verify volume %v is attached to the pod %v", volumePath, nodeName))
		...
		for i, pod := range pods {
			...
			By(fmt.Sprintf("Waiting for volume %v to be detached from the node %v", volumePath, nodeName))
			err = waitForVSphereDiskToDetach(vsp, volumePath, types.NodeName(nodeName))
			Expect(err).NotTo(HaveOccurred())

			By(fmt.Sprintf("Deleting volume %v", volumePath))
