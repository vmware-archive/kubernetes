...
var _ = SIGDescribe("PersistentVolumes:vsphere", func() {
	...
	*/
	BeforeEach(func() {
		...
		By("Creating the Client Deployment")
		deployment, err = framework.CreateDeployment(c, int32(1), map[string]string{"test": "app"}, ns, pvc, "")
		Expect(err).NotTo(HaveOccurred())
	...
	*/
	...
	/*
		This test verifies that a volume attached to a pod after kubelet stop are attached to other available nodes where the pod got rescheduled.
		Steps:
		1. Create a pod on a node.
		2. Verify volume is attached on the node.
		3. Stop kubelet.
		4. Verify the pod gets provisioned on other available node.
		5. Verify the volume is attached to new node.
	*/
	It("should test that a file written to the vspehre volume mount before kubelet restart can be read after restart [Disruptive]", func() {
		testKubeletStopAndVerifyVolumeMounts(c, f, clientPod, pvc, pv, vsp)
	})
})
